...
이 문서는 `도면자동화 v0.8.0`의 현재 구현을 기준으로 작성한 상세 기술문서다.
...
- 제품 표시 버전: `v0.8.0`
- npm 패키지명: `drawing-automation-v0.8.0`
- npm 패키지 버전: `0.8.0`
- 층별 장비 분석 규칙 버전: `23`
- IndexedDB 스키마 버전: `1`
...
`ANALYSIS_VERSION`은 현재 `23`이다. 저장된 도면의 버전이 다르면 도면 선택 시 자동으로 다시
...
`v0.8.0` 수준이다. `v1.0.0`으로 올리기 전에는 장비 중복 제거와 범위 태그 확장, 문서 검토
정확도, 대용량 API 스트리밍, 실제 사용자 시나리오 기반 장기 검증을 우선 완료하는 것이 적절하다.
