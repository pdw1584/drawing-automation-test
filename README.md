...
- 등록 도면 헤더의 수정 모드에서 표시 이름·설명 수정과 위아래 순서 변경 및 영구 저장
- 장비 목록과 장비 종류별 필터 버블 제공
- 화면에서 사용자 자동 검출 장비 분류와 약어를 등록·삭제하고 재분석에 적용
- 사용자 검출 장비 사전 JSON 내보내기·병합/교체 가져오기 및 이전 사전 복구
- 미연결·저신뢰·중복 의심 항목 전용 검수 필터와 판넬 매칭 거리 표시
- 목록 항목 다중 선택 후 일괄 제외·복원 또는 확인 완료 처리
- 장비별 `미검토·확인 완료·수정 필요` 검수 상태 저장과 상태별 필터
- 확인 완료 장비는 중복 의심·미연결·저신뢰 필터에서 숨기고 미검토 전환 시 다시 표시
- 장비를 클릭하여 도면상의 해당 위치로 이동
- 자동 판넬명을 사용자가 수정하거나 연결 해제하고 오검출 장비를 제외
- 사용자 교정값을 자동 재분석 이후에도 우선 적용하고 자동 결과로 복원 가능
- 전체 등록 도면의 층별·장비 분류별 수량 집계
- 장비 상세 목록과 수량 집계를 두 시트 Excel 파일로 내보내기
- 내보낸 Excel 장비목록에서 수정한 판넬명과 검수 상태 다시 가져오기
- 분석 결과와 원본 DXF를 브라우저 IndexedDB에 저장
- 분석 규칙이 갱신되면 저장된 도면을 자동 재분석
- 등록된 모든 도면을 진행률과 성공·실패 결과를 확인하며 일괄 재분석
...
BATT & DC, Load Bank, MOF, RTU 등이 포함됩니다. Load Bank는 `LOAD BANK`, `부하시험기`,
`로드 뱅크`, `로드뱅크` 표기를 모두 인식합니다.
